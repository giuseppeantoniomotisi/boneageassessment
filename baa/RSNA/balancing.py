...
optionally, generating augmented images. Here's a summary of what each part of the code does:

• Initialization:
The class is initialized with default paths and dependencies extracted from an initialization module
(__init__).
It sets paths for CSV files, raw images, and the processed training folder.
It reads the training CSV file into a Pandas DataFrame and adds an "angle" column filled with zeros.
find_element(limit_value, num_classes) method:
This method categorizes elements in the DataFrame into classes based on their values and calculates
delta values based on the histogram of key_values column and limit_value.
The result is a tuple containing a dictionary representing classes and an array representing
the delta values.
• create_bal_csv() method:
This method creates a balanced CSV file by augmenting the training data with additional samples.
It uses the find_element method to categorize elements into classes and calculates delta values.
Additional samples are generated by randomly selecting images, adding a rotation angle, and
creating new entries in the CSV file.
The augmented CSV file is saved.
• rotation(image, angle) static method:
This static method rotates an image by a given angle.
• create_images() method:
This method generates augmented images based on the balanced CSV file and saves them to the
processed training folder.
It reads the augmented CSV file, and for each entry, it either copies the original image or
rotates it based on the specified angle.
• balance(create_images: bool = False) method:

...
>[!IMPORTANT]
>Before using this script, ensure that you have already installed the *dataset.zip*(only if RSNA flag is `false`) and *weights.zip* files at following link to [Google Drive](https://drive.google.com/drive/folders/1zNPHIJymBkvtQQkJgFwa-7K-x1XH2qdp). Once downloaded, verify if both files are present in the 'Downloads' directory, and then please proceed to unzip them.
```json
/*macro.json*/
